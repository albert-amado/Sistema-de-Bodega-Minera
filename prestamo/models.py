    ...
    def clean(self):
        if not self.documento.isdigit():
            raise ValidationError(
                {"documento": "El documento debe contener solo dígitos."}
            )
...
class DetallePrestamo(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.cantidad}x {self.herramienta.nombre} "
            f"(Préstamo #{self.prestamo_id})"
        )

    def clean(self):
        if (
            self.herramienta
            and self.cantidad > self.herramienta.stock_disponible
        ):
            msg = (
                f"Stock insuficiente en modelo. Solo hay "
                f"{self.herramienta.stock_disponible} disponibles."
            )
