...
from .models import Herramienta, Prestamo, DetallePrestamo, DevolucionHerramienta, EstadoPrestamo


class HumanForm(forms.ModelForm):
    """Clase base para aplicar estilos CSS y comportamientos amigables automáticamente."""
...
class HerramientaForm(HumanForm):
    class Meta:
        model = Herramienta
        fields = ['codigo', 'nombre', 'descripcion', 'stock_disponible']
        labels = {'codigo': 'Código / Placa', 'stock_disponible': 'Stock Disponible'}
...
class PrestamoForm(HumanForm):
    class Meta:
        ...
        labels = {'documento': 'Documento Solicitante', 'ficha': 'Ficha SENA / Programa'}
...
class DetallePrestamoForm(HumanForm):
    ...
    def clean(self):
        ...
        if h and cant and cant > h.stock_disponible:
            self.add_error('cantidad', f"Stock insuficiente. Solo hay {h.stock_disponible} disponibles.")
        return cd


DetallePrestamoFormSet = inlineformset_factory(
    Prestamo, DetallePrestamo, form=DetallePrestamoForm, extra=1, can_delete=True
)
...
class DevolucionHerramientaForm(HumanForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk and not self.initial.get('fecha'):
            self.initial['fecha'] = timezone.now().strftime('%Y-%m-%d')
        self.fields['prestamo'].queryset = Prestamo.objects.exclude(estado=EstadoPrestamo.DEVUELTO)
