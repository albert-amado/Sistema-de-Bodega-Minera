...
def check_ui_structure():
    ...
    if total == 0:
        return {
            "status": "RED",
            "score": 0,
            "msg": "No se encontraron archivos HTML/Plantillas.",
            "details": [],
        }
...
def check_api_consumption():
    ...
    loading_pattern = re.compile(
        r"Cargando|cargando|loading|spinner|spinner-border|disabled\s*=\s*true"
    )

    py_request_pattern = re.compile(
        r"\brequests\.(get|post|put|delete|patch)\s*\("
    )
    py_except_pattern = re.compile(
        r"except\s+.*RequestException|except\s+Exception"
    )
    ...
    for det in api_detections:
        ...
        loading_txt = (
            "y estado de carga"
            if det["loading_state"]
            else "sin estado de carga"
        )
        details.append(
            f"  - {det['file']} ({det['type']}): {det['calls']} llamadas ({status_txt} {loading_txt})"
        )
        if det["handled"]:
            handled_calls += 1
    ...
    if not api_detections:
        return {
            ...
            "details": [
                "Considera integrar servicios backend JSON REST si el proyecto lo requiere."
            ],
        }
...
def check_styles_bem():
    ...
    bem_pattern = re.compile(
        r"\.[a-zA-Z0-9_-]+__[a-zA-Z0-9_-]+|\.[a-zA-Z0-9_-]+--[a-zA-Z0-9_-]+"
    )
    inline_style_pattern = re.compile(r'\bstyle\s*=\s*["\'][^"\']*["\']')
    bootstrap_class_pattern = re.compile(
        r'\bclass\s*=\s*["\'][^"\']*(col-|row|d-flex|d-grid|bg-|text-|shadow-|rounded-|p-|m-)[^"\']*["\']'
    )
            ...
            if f.endswith(".css"):
                ...
                try:
                    ...
                    with open(path, "r", encoding="utf-8") as file:
                        ...
                        content = "".join(lines)
                        bootstrap_utilities += len(
                            bootstrap_class_pattern.findall(content)
                        )
...
def check_data_binding():
    ...
    listener_patterns = [
        re.compile(
            r'\.addEventListener\s*\(\s*[\'"](change|input|keyup|click|submit)[\'"]'
        ),
...
def check_env_variables():
    ...
    """
    env_exists = os.path.exists(
        os.path.join(BASE_DIR, ".env")
    ) or os.path.exists(os.path.join(BASE_DIR, ".env.example"))
    os_getenv_count = 0
    decouple_count = 0
    dotenv_config_detected = False
    hardcoded_secrets = []

    secret_key_hardcoded = re.compile(
        r'SECRET_KEY\s*=\s*[\'"][^\'"]*django-insecure[^\'"]*[\'"]'
    )
    email_pass_hardcoded = re.compile(
        r'EMAIL_HOST_PASSWORD\s*=\s*[\'"](?![$\{\s])[^\'"]+[\'"]'
    )
            ...
            if f.endswith(".py"):
                try:
                    with open(path, "r", encoding="utf-8") as file:
                        content = file.read()
                        os_getenv_count += len(
                            re.findall(r"os\.getenv|os\.environ", content)
                        )
                        decouple_count += len(
                            re.findall(r"\bconfig\s*\(", content)
                        )

                        if (
                            "load_dotenv" in content
                            or "dotenv.load_dotenv" in content
                        ):
                        ...
                        if f == "settings.py":
                            ...
                            if email_pass_hardcoded.search(content):
                                hardcoded_secrets.append(
                                    "EMAIL_HOST_PASSWORD expuesta en settings.py"
                                )
                except Exception:
                    pass
...
def check_hierarchy():
    ...
    has_mvt = (
        categories["modelos (models.py)"] > 0
        and categories["vistas (views.py)"] > 0
        and categories["plantillas (.html)"] > 0
    )
...
def check_unit_tests():
    ...
    case_pat = re.compile(
        r"class\s+(?:\w*(?:TestCase|Tests|Test)\w*\s*\(|\w+\s*\([^)]*(?:TestCase|Tests|Test))"
    )
            ...
            if f.endswith(".html") and f != "reporte_rubrica.html":
                ...
                try:
                    with open(path, "r", encoding="utf-8") as file:
                        ...
                        defer_async_scripts += len(
                            script_opt_pattern.findall(content)
                        )
                except Exception:
                    pass
...
def check_responsiveness():
    ...
    grid_pattern = re.compile(
        r'\bclass\s*=\s*["\'][^"\']*(col-|row|d-flex|d-grid|flex-wrap|col-sm-|col-md-|col-lg-)[^"\']*["\']'
    )
...
"""
    ...
    sugerencias = [
        p["suggestion"]
        for p in resultados
        if p["score"] < 100 and p.get("suggestion")
    ]
    if sugerencias:
        sugerencias_items = "\n".join(
            [f"            <li>{sug}</li>" for sug in sugerencias]
        )
        sugerencias_html = f"""
...
"""
    ...
    try:
        ...
        print(
            "Para generar el PDF directamente, instala xhtml2pdf: pip install xhtml2pdf"
        )
        print(
            "Tambien puedes abrir 'reporte_rubrica.html' en tu navegador y guardarlo como PDF."
        )
    except Exception as e:
...
def main():
    print("=" * 70)
    print(
        f"{BOLD}{WHITE}AUDITORÍA TÉCNICA DE CALIDAD - PROYECTO {PROJECT_NAME.upper()}{RESET}"
    )
    print(
        "Evaluando cumplimiento de los 12 puntos de desarrollo frontend/backend..."
    )
    print("=" * 70)

    puntos = [
        (
            "1. Estructura de Componentes de Interfaz (Plantillas)",
            check_ui_structure,
        ),
    ...
    if average_score >= 90:
        ...
        print_colored(
            "  ESTADO FINAL: EXCELENTE / LISTO PARA ENTREGA ✔", GREEN
        )
    elif average_score >= 70:
        estado_final_lbl = "ACEPTABLE CON DETALLES POR CORREGIR"
        estado_final_status = "YELLOW"
        print_colored(
            "  ESTADO FINAL: ACEPTABLE CON DETALLES POR CORREGIR ⚠", YELLOW
        )
    else:
        estado_final_lbl = "REQUIERE MEJORAS CRÍTICAS ANTES DE LA ENTREGA"
        estado_final_status = "RED"
        print_colored(
            "  ESTADO FINAL: REQUIERE MEJORAS CRÍTICAS ANTES DE LA ENTREGA ✖",
            RED,
        )

    estado_final = {"name": estado_final_lbl, "status": estado_final_status}

    print("=" * 70)
    print("Sugerencias clave:")
    sugerencias = [
        p["suggestion"]
        for p in resultados_reporte
        if p["score"] < 100 and p.get("suggestion")
    ]
    if sugerencias:
        ...
        print_colored(
            "  ¡Excelente! Todos los puntos de control están al 100%. No hay sugerencias pendientes.",
            GREEN,
        )
    print("=" * 70)
